...
static void generate_assignment_statement ( node_t *statement )
{
    ...
    if (lhs->type == IDENTIFIER_DATA) {
        ...
        switch (symbol->type) {
            ...
            default:
                break;
                // fprintf(stderr, "error: unsupported symbol type for assignment\n");
                // exit(EXIT_FAILURE);
        }
    }
        ...
        //  else {
        //     fprintf(stderr, "error: unsupported symbol type for array indexing\n");
        //     exit(EXIT_FAILURE);
        // }
        IMULQ(IMM(8), RAX);
        ADDQ(RAX, RCX);
...
static void generate_print_statement ( node_t *statement )
{
    ...
    for (size_t i = 0; i < statement->n_children; ++i) {
        ...
        if (item->type == STRING_LIST_REFERENCE) {
            ...
            // DIRECTIVE("string_%s", i + ":");
            DIRECTIVE(".asciz %s\n", *(char*)item->data + "\"");
            // LEAQ(ARRAY_MEM(".string_" + i, "rip", "1"), RSI);
            LEAQ("$strout(%rip)", RDI);
            char data[50];
