...
assignment_statement :
      identifier ':' '=' expression {
          $$ = node_create ( ASSIGNMENT_STATEMENT, NULL, 2, $1, $4 );
      }
...
expression :
    ...
    | expression '<' '<' expression {
          $$ = node_create ( EXPRESSION, "<<", 2, $1, $4);
      }
    | expression '>' '>' expression {
          $$ = node_create ( EXPRESSION, ">>", 2, $1, $4 );
      }
    | '-' expression %prec UMINUS {
          $$ = node_create ( EXPRESSION, "-", 1, $2 );
      }
    | '(' expression ')' { $$ = $2; }
    | number { $$ = $1; }
    | identifier { $$ = $1; }
...
function_call :
      identifier '(' argument_list ')' {
          $$ = node_create ( FUNCTION_CALL, NULL, 2, $1, $3 );
      }
    ;
...
argument_list :
      expression_list { $$ = $1; }
    | /* Empty */ {
          $$ = node_create ( LIST, NULL, 0 );
      }
    ;

expression_list :
      expression {
          $$ = node_create ( LIST, NULL, 1, $1 );
      }
...
identifier :
      IDENTIFIER {
          $$ = node_create ( IDENTIFIER_DATA, strdup(yytext), 0 );
      }
    ;

number :
      NUMBER {
          $$ = node_create ( NUMBER_DATA, strdup(yytext), 0 );
      }
    ;

string :
      STRING {
          $$ = node_create ( STRING_DATA, strdup(yytext), 0 );
      }
    ;
